...
return new class extends Migration
{
    ...
    {
        DB::unprepared("CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_actualizar_evento`(IN `p_calendario_id` INT, IN `p_titulo` VARCHAR(100), IN `p_descripcion` TEXT, IN `p_tipo_evento` ENUM('Virtual','Presencial','InicioProyecto','FinProyecto'), IN `p_estado_evento` ENUM('Programado','EnCurso','Finalizado'), IN `p_fecha_inicio` DATETIME, IN `p_fecha_fin` DATETIME, IN `p_hora_inicio` TIME, IN `p_hora_fin` TIME, IN `p_ubicacion` VARCHAR(200), IN `p_organizador_id` INT, IN `p_proyecto_id` INT, OUT `p_mensaje` VARCHAR(255))
BEGIN
