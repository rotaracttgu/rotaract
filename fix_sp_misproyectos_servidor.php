<?php
/**
 * Arreglar SP_MisProyectos - cambiar m_resp.Nombre por u_resp.name
 */
...
$sql = <<<'SQL'
CREATE DEFINER=`dbadmin`@`localhost` PROCEDURE `SP_MisProyectos`(IN `p_user_id` BIGINT, IN `p_filtro_estado` VARCHAR(50), IN `p_filtro_tipo` VARCHAR(50), IN `p_buscar` VARCHAR(255))
BEGIN
...
DB::statement("DROP PROCEDURE IF EXISTS SP_MisProyectos");
DB::statement($sql);

echo "✅ SP_MisProyectos corregido\n";
