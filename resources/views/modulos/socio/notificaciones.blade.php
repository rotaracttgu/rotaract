...
<div class="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100">
    <div class="max-w-7xl mx-auto">
        ...
        <div class="mb-6 flex justify-between items-center">
            ...
            <a href="{{ route('socio.dashboard') }}" class="bg-gradient-to-r from-gray-600 to-gray-700 hover:from-gray-700 hover:to-gray-800 text-white font-bold py-2 px-4 rounded-lg shadow-md hover:shadow-lg transform hover:-translate-y-0.5 transition-all duration-200 flex items-center gap-2">
        ...
        <div class="bg-white rounded-xl shadow-lg p-6 border border-gray-100 mb-6">
            <div class="flex flex-wrap gap-4 items-center justify-between">
                <div class="flex gap-3">
                    <button onclick="filterNotifications('todas')" class="px-4 py-2 rounded-lg font-medium transition filter-btn active bg-gradient-to-r from-blue-600 to-green-600 text-white" data-filter="todas">
                        Todas
                    </button>
            ...
            <div class="p-6">
                ...
                <div id="notificationsList" class="space-y-3">
                    @forelse($notificaciones as $notificacion)
                        @php
                            // Determinar enlace según tipo de notificación
                            $enlace = '#';
                            
                            if(str_contains($notificacion->tipo, 'reunion')) {
                                $enlace = route('socio.calendario');
                            } elseif(str_contains($notificacion->tipo, 'proyecto')) {
                                $enlace = route('socio.proyectos');
                            }
                        @endphp
...
<script>
    function filterNotifications(tipo) {
        ...
        buttons.forEach(btn => {
            if (btn.dataset.filter === tipo) {
                ...
                btn.classList.add('bg-gradient-to-r', 'from-blue-600', 'to-green-600', 'text-white');
            } else {
                btn.classList.remove('active', 'bg-gradient-to-r', 'from-blue-600', 'to-green-600', 'text-white');
    ...
    function markAsRead(id) {
        fetch(`/socio/notificaciones/${id}/marcar-leida`, {
            method: 'POST',
            headers: {
    ...
    function markAllAsRead() {
        if (confirm('¿Marcar todas las notificaciones como leídas?')) {
            fetch('/socio/notificaciones/marcar-todas-leidas', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
