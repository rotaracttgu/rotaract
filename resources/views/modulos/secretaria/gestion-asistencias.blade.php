...
<head>
    ...
    <title>Secretaría - Gestión de Asistencias</title>
...
<body>
    <div class="container-fluid">
        <div class="sidebar">
            ...
            <nav class="sidebar-nav">
                ...
                <a class="nav-link {{ request()->routeIs('secretaria.calendario') ? 'active' : '' }}" href="{{ route('secretaria.calendario') }}">
                    <i class="fas fa-calendar"></i> Calendario
                </a>
                <a class="nav-link {{ request()->routeIs('secretaria.eventos') ? 'active' : '' }}" href="{{ route('secretaria.eventos') }}">
                    <i class="fas fa-calendar-plus"></i> Gestión de Eventos
                </a>
                <a class="nav-link {{ request()->routeIs('secretaria.asistencias') ? 'active' : '' }}" href="{{ route('secretaria.asistencias') }}">
                    <i class="fas fa-users"></i> Asistencias
                </a>
                <a class="nav-link {{ request()->routeIs('secretaria.reportes') ? 'active' : '' }}" href="{{ route('secretaria.reportes') }}">
    ...
    <script>
        ...
        function loadMiembros() {
            $.ajax({
                url: '/api/secretaria/calendario/miembros',
                method: 'GET',
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                },
        ...
        function loadAttendanceData(eventId = currentEventId) {
            ...
            $.ajax({
                url: `/api/secretaria/calendario/eventos/${eventId}/asistencias`,
                method: 'GET',
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                },
        ...
        function saveAttendance() {
            ...
            if (isEdit) {
                ...
                $.ajax({
                    url: `/api/secretaria/calendario/asistencias/${attendanceId}`,
                    method: 'PUT',
                    headers: {
                        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content'),
                ...
                $.ajax({
                    url: '/api/secretaria/calendario/asistencias',
                    method: 'POST',
                    headers: {
                        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content'),
                        'Content-Type': 'application/json'
        ...
        function deleteAttendance(id) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        url: `/api/secretaria/calendario/asistencias/${id}`,
                        method: 'DELETE',
                        headers: {
                            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                        },
